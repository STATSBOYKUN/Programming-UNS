...
data = pd.read_csv("RegresiMeninggal.csv")
data.head()

print('loading tunggu sabar bos tergantung devicenya...')
# data['Meninggal'] = data['Meninggal'].str.replace(',', '')
# data['Hari'] = data['Hari'].str.replace(',', '')

data['Meninggal'] = pd.to_numeric(data['Meninggal'])
...
y = data['Meninggal'].values.reshape(-1,1)
